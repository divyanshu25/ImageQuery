...
class EncoderKWTL(nn.Module):
    def __init__(self, embed_size):
        ...
        self.fine_tune(
            status=config.train_encoder
        )  # To fine-tune the CNN, self.fine_tune(status = True)
    ...
    def forward(self, images):
        ...
        enc_image = enc_image.view(
            batch_size, num_pixels, features
    ...
    def fine_tune(self, status=False):
        if not status:
            ...
            for module in list(self.resnet.children())[
                7:
    ...
    def get_learning_parameters(self):
        ...
        if config.train_encoder:
            params = params + list(
                self.resnet[7].parameters()
            )  # only the last layer's parmaeters
        return params
