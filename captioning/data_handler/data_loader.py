...
def get_data_loader_common(config, mode='train', type='flickr8k'):
    data_loader = None

    if type == 'flickr8k':
        flickr_ann_dict = parse_flickr(Config.annotations_file)
        data_loader = get_data_loader(config, flickr_ann_dict, mode)
...
def get_coco_data_loader(config, mode='train'):
    ...
    if mode == 'train':
        trainset = dset.CocoCaptions(root=config.train_root_dir_coco,
                                    annFile=config.train_ann_file_coco,
                                    transform=transform)
        indices = range(0, 30000)
        dataset = Subset(trainset, indices)

    elif mode == 'val':
        dataset = dset.CocoCaptions(root=config.val_root_dir_coco,
                                    annFile=config.val_ann_file_coco,
                                    transform=transform)

    elif mode == 'test':
        dataset = dset.CocoCaptions(root=config.test_root_dir_coco,
